"""Test fixtures for shipping service tests."""
...
def db_engine() -> Any:
    ...
    from services.shipping.models import models as _shipping_models  # noqa: F401
...
@pytest.fixture()
def sample_shipment(
    db_session: Session, sample_account: Account, sample_order: Order
) -> Shipment:
...
@pytest.fixture()
def sample_item2(db_session: Session, sample_account: Account, sample_category: Category) -> InventoryItem:
...
@pytest.fixture()
def sample_order2(
    db_session: Session, sample_account: Account, sample_show: Show, sample_item2: InventoryItem
) -> Order:
    """Create a second order for bulk testing."""
    order = Order(
        account_id=sample_account.id,
        show_id=sample_show.id,
        inventory_item_id=sample_item2.id,
        sale_price=15.00,
        platform_fees=1.50,
        shipping_cost=3.00,
        cost_basis=float(sample_item2.cogs),
        profit=15.00 - 1.50 - 3.00 - float(sample_item2.cogs),
        buyer_username="buyer456",
    )
    sample_item2.status = ItemStatus.SOLD
    db_session.add(order)
    db_session.flush()
    return order
...
def app(db_engine: Any) -> Flask:
    ...
    test_app.config["_SHIPPING_PROVIDER"] = None
    JWTManager(test_app)
    return test_app
